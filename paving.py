...
class Paving:
    ...
    def load_mnf(self,filename,bxtyp=[0,1,2,3]):
        ...
        with open(filename,'rb') as f:
            ...
            assert (entete==self.entete and version<=self.version), 'bad version input file'
    ...
    def save_mnf(self,filename):
        ...
        with open(filename,'wb') as f:
            ...
            # encoding preamble
            writestr(self.entete)
            writeint(2)
            print(" Writing", str(self.entete), "version", 2, " to ",filename)
    ...
    def from_covfile(self,filename,bxtyp=[0,1,2,3]):
        '''
        Fills the Paving object according to a given Cov object (see COVreader for more information)
        '''
        cov = COVreader.Cov(filename) # load COV file using COVreader
        # Converting general data
        self.entete = cov.format.signature
        self.version = cov.format.level     # not an exact correspondence
        self.data['nvar'] = cov.dim         # number of variables
        self.data['nce']  = cov.neqs        # number of constraint equations
        self.data['nci']  = cov.nineqs      # number of constraint inequalities
        self.data['sta']  = cov.search_status # computation status
        self.data['nbox'] = cov.nboxes      # number of boxes
        self.data['nin']  = cov.ninners+cov.nsols     # number of inner boxes
        self.data['nbd']  = cov.manifold_nboundaries # number of boundary boxes
        self.data['npd']  = cov.npendings   # number of pending boxes
        self.data['nun']  = cov.nboxes - cov.ninners - cov.nsols - cov.manifold_nboundaries - cov.npendings # number of unknown boxes
        self.data['time'] = cov.time        # computation time
        self.data['ncel'] = cov.ncells      # number of cells
        # Converting boxes
        self.boxes=[]
        p = rtree.index.Property()
        p.dimension = self.data['nvar']
        self._rtree = rtree.index.Index(properties=p,interleaved=False)
        inners = sorted(set(cov.inners+[s["index"] for s in cov.sols]))
        boundaries = sorted(set([b["index"] for b in cov.manifold_boundaries]))
        for i,b in enumerate(cov.boxes):
            print("   box {}/{}\r".format(i+1,self.data['nbox']),end='\r')
            # box coordinates
            vec = []
            for itv in b:
                vec.append(itv.lower_bound) ; vec.append(itv.upper_bound)
            # box type (0=inner, 1=boundary, 2=unknown, 3=pending)
            if i in inners:
                typ = 0
            elif i in boundaries:
                typ = 1
            elif i in cov.pendings:
                typ = 3
            else:
                typ = 2
            # certificate parameters : neglected
            par = None
            if typ in bxtyp:
                self.boxes.append(Box(i,vec,{'type':typ,'parameters':par}))     # pushing the converted box in the paving
                self._rtree.insert(i,vec)
        print("")
        # updating number of boxes depending on selected types
        if not 0 in bxtyp:
